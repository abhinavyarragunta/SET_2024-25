...
SoftwareSerial Serial1 (0, 1); // define the soft serial port as Serial1, pin2 as RX, and pin3 as TX
...
void loop()
{
  if (Serial1.available())//check whether the serial port has data input
  {
    if(Serial1.read()==HEADER)// determine data package frame header 0x59
    {
      ...
      {
        uart[1]=HEADER;
        for(i=2;i<9;i++)// store data to array, reading bytes 2-8
        {
          uart[i]=Serial1.read();
        }
        check=uart[0]+uart[1]+uart[2]+uart[3]+uart[4]+uart[5]+uart[6]+uart[7]; // checking checksum
        if(uart[8]==(check&0xff))// check the received data as per protocols
        {
          dist=uart[2]+uart[3]*256;// calculate distance value, converting low and high bytes
          strength=uart[4]+uart[5]*256;// calculate signal strength value, converting low and high bytes
          Serial.print("dist = ");
